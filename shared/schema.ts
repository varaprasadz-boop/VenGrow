...
export const properties = pgTable("properties", {
  ...
  superBuiltUpArea: integer("super_built_up_area"),
  carParkingCount: integer("car_parking_count"),
  maintenanceCharges: integer("maintenance_charges"),
  viewType: text("view_type"),
  numberOfLifts: integer("number_of_lifts"),
  isNegotiable: boolean("is_negotiable"),
  securityDeposit: integer("security_deposit"),
  lockInMonths: integer("lock_in_months"),
  tenantsPreferred: text("tenants_preferred"),
  negotiableRent: text("negotiable_rent"),
  brokerageBothSides: text("brokerage_both_sides"),
  ...
  newProjectCategory: text("new_project_category"),
  newProjectFloorPlans: jsonb("new_project_floor_plans").$type<{ superBuiltUpArea?: number; carpetArea?: number; bhk?: number; bathrooms?: number; balconies?: number; totalPrice?: number }[]>(),
  newProjectDimensions: jsonb("new_project_dimensions").$type<{ area?: number; totalPrice?: number }[]>(),
  areaInLocality: text("area_in_locality"),
  projectSocietyName: text("project_society_name"),
  overlookingType: text("overlooking_type"),
  landArea: integer("land_area"),
  roomSizes: jsonb("room_sizes").$type<{ room: string; size: string }[]>(),
  pgSharingPricing: jsonb("pg_sharing_pricing").$type<{ type: string; rent: number; deposit: number }[]>(),
...
export const formFieldTypeEnum = pgEnum("form_field_type", [
  "text", "numeric", "alphanumeric", "textarea", "checkbox", "dropdown", "radio", "date", "file_upload", "map"
]);

export const formFields = pgTable("form_fields", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  sectionId: varchar("section_id").notNull().references(() => formSections.id),
  label: text("label").notNull(),
  ...
  isDefault: boolean("is_default").notNull().default(false),
  sortOrder: integer("sort_order").notNull().default(0),
  validationRules: jsonb("validation_rules").$type<{ min?: number; max?: number; charLimit?: number; regex?: string }>(),
  options: jsonb("options").$type<string[]>(),
  fileConfig: jsonb("file_config").$type<{ maxFiles?: number; maxSizeMB?: number; allowedTypes?: string[] }>(),
  sourceType: text("source_type"),
  linkedFieldKey: text("linked_field_key"),
  defaultValue: text("default_value"),
  displayStyle: text("display_style").default("default"),
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});

export const propertyCustomData = pgTable("property_custom_data", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  propertyId: varchar("property_id").notNull().references(() => properties.id),
  formTemplateId: varchar("form_template_id").notNull().references(() => formTemplates.id),
...
export const insertPropertyCustomDataSchema = createInsertSchema(propertyCustomData).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type InsertPropertyCustomData = z.infer<typeof insertPropertyCustomDataSchema>;
export type PropertyCustomData = typeof propertyCustomData.$inferSelect;
