import { 
  ...
  type FormSection, type InsertFormSection,
  type FormField, type InsertFormField,
  type PropertyCustomData, type InsertPropertyCustomData,
  ...
  formTemplates, formSections, formFields, propertyCustomData
} from "@shared/schema";
...
export interface IStorage {
  ...
  // Form Templates (T005)
  getFormTemplates(filters?: { sellerType?: string; status?: string }): Promise<FormTemplate[]>;
  getFormTemplate(id: string): Promise<(FormTemplate & { sections: (FormSection & { fields: FormField[] })[] }) | undefined>;
  ...
  updateFormSection(id: string, data: Partial<InsertFormSection>): Promise<FormSection | undefined>;
  deleteFormSection(id: string): Promise<boolean>;
  reorderFormSections(templateId: string, orderedIds: string[]): Promise<void>;
  duplicateFormSection(id: string): Promise<FormSection | undefined>;
  ...
  reorderFormFields(sectionId: string, orderedIds: string[]): Promise<void>;

  getFilterableFormFields(): Promise<Array<{
    sectionName: string;
  ...
  // Property Custom Data (T020)
  createPropertyCustomData(data: InsertPropertyCustomData): Promise<PropertyCustomData>;
  getPropertyCustomData(propertyId: string): Promise<PropertyCustomData | undefined>;
  updatePropertyCustomData(propertyId: string, data: Partial<InsertPropertyCustomData>): Promise<PropertyCustomData | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getFormTemplates(filters?: { sellerType?: string; status?: string }): Promise<FormTemplate[]> {
    const conditions = [];
    if (filters?.sellerType) {
      conditions.push(eq(formTemplates.sellerType, filters.sellerType as any));
    }
    if (filters?.status) {
      conditions.push(eq(formTemplates.status, filters.status as any));
    }
    let query = db.select().from(formTemplates);
    if (conditions.length > 0) {
      query = query.where(and(...conditions)) as any;
    }
    return query.orderBy(desc(formTemplates.updatedAt));
  }

  async getFormTemplate(id: string): Promise<(FormTemplate & { sections: (FormSection & { fields: FormField[] })[] }) | undefined> {
    ...
    const sections = await db.select().from(formSections)
      ...
      .orderBy(formSections.stage, formSections.sortOrder);

    const sectionIds = sections.map(s => s.id);
    let fields: FormField[] = [];
    if (sectionIds.length > 0) {
      fields = await db.select().from(formFields)
  ...
  async createFormTemplate(data: InsertFormTemplate): Promise<FormTemplate> {
    const [created] = await db.insert(formTemplates).values(data as any).returning();
    return created;
  }
  ...
  async cloneFormTemplate(id: string): Promise<FormTemplate | undefined> {
    const original = await this.getFormTemplate(id);
    if (!original) return undefined;

    const maxVersion = await db.select({ maxVer: sql<number>`COALESCE(MAX(${formTemplates.version}), 0)` })
      .from(formTemplates)
      .where(eq(formTemplates.sellerType, original.sellerType));
    const newVersion = (maxVersion[0]?.maxVer || 0) + 1;

    const [cloned] = await db.insert(formTemplates).values({
      name: `${original.name} (Copy)`,
      sellerType: original.sellerType,
      version: newVersion,
      status: "draft" as any,
      assignedCategories: original.assignedCategories,
      showPreviewBeforeSubmit: original.showPreviewBeforeSubmit,
      allowSaveDraft: original.allowSaveDraft,
      autoApproval: original.autoApproval,
      termsText: original.termsText,
      seoMetaTitle: original.seoMetaTitle,
      seoMetaDescription: original.seoMetaDescription,
    }).returning();

    for (const section of original.sections) {
      const [newSection] = await db.insert(formSections).values({
        formTemplateId: cloned.id,
        stage: section.stage,
        name: section.name,
        icon: section.icon,
      ...
      for (const field of section.fields) {
        await db.insert(formFields).values({
          ...
          linkedFieldKey: field.linkedFieldKey,
  ...
  async getPublishedFormForSeller(sellerType: string): Promise<(FormTemplate & { sections: (FormSection & { fields: FormField[] })[] }) | undefined> {
  ...
  async createFormSection(data: InsertFormSection): Promise<FormSection> {
    const [created] = await db.insert(formSections).values(data as any).returning();
    return created;
  }

  async updateFormSection(id: string, data: Partial<InsertFormSection>): Promise<FormSection | undefined> {
    const [updated] = await db.update(formSections)
      .set(data as any)
  ...
  async deleteFormSection(id: string): Promise<boolean> {
    await db.delete(formFields).where(eq(formFields.sectionId, id));
    await db.delete(formSections).where(eq(formSections.id, id));
    return true;
  }

  async reorderFormSections(templateId: string, orderedIds: string[]): Promise<void> {
    for (let i = 0; i < orderedIds.length; i++) {
      await db.update(formSections)
        .set({ sortOrder: i })
        .where(and(eq(formSections.id, orderedIds[i]), eq(formSections.formTemplateId, templateId)));
    }
  }

  async duplicateFormSection(id: string): Promise<FormSection | undefined> {
    const [original] = await db.select().from(formSections).where(eq(formSections.id, id));
    if (!original) return undefined;

    const fields = await db.select().from(formFields)
      .where(eq(formFields.sectionId, id))
      .orderBy(formFields.sortOrder);
    ...
    const [newSection] = await db.insert(formSections).values({
      ...
      showInFilters: original.showInFilters,
      isDefault: false,
  ...
  async createFormField(data: InsertFormField): Promise<FormField> {
    const [created] = await db.insert(formFields).values(data as any).returning();
    return created;
  }
  ...
  async reorderFormFields(sectionId: string, orderedIds: string[]): Promise<void> {
    for (let i = 0; i < orderedIds.length; i++) {
      await db.update(formFields)
        .set({ sortOrder: i })
        .where(and(eq(formFields.id, orderedIds[i]), eq(formFields.sectionId, sectionId)));
    }
  }

  async getFilterableFormFields(): Promise<Array<{
    sectionName: string;
    sectionIcon: string | null;
    fields: Array<{
      label: string;
    ...
    const sections = await db.select({
      sectionId: formSections.id,
      sectionName: formSections.name,
      sectionIcon: formSections.icon,
      sectionSortOrder: formSections.sortOrder,
    })
      .from(formSections)
      .innerJoin(formTemplates, eq(formSections.formTemplateId, formTemplates.id))
      .where(and(
        eq(formSections.showInFilters, true),
        eq(formSections.isActive, true),
        eq(formSections.stage, 2),
        eq(formTemplates.status, "published" as any),
      ))
      .orderBy(formSections.sortOrder);
    ...
    for (const section of sections) {
      if (seen.has(section.sectionName)) continue;
      seen.set(section.sectionName, result.length);
  ...
  async createPropertyCustomData(data: InsertPropertyCustomData): Promise<PropertyCustomData> {
    const [created] = await db.insert(propertyCustomData).values(data as any).returning();
    return created;
  }

  async getPropertyCustomData(propertyId: string): Promise<PropertyCustomData | undefined> {
    const [result] = await db.select().from(propertyCustomData)
      .where(eq(propertyCustomData.propertyId, propertyId));
    return result;
  }

  async updatePropertyCustomData(propertyId: string, data: Partial<InsertPropertyCustomData>): Promise<PropertyCustomData | undefined> {
    const [updated] = await db.update(propertyCustomData)
      .set({ ...data, updatedAt: new Date() } as any)
      .where(eq(propertyCustomData.propertyId, propertyId))
      .returning();
    return updated;
